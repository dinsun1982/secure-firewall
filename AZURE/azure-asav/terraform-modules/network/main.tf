...
resource "azurerm_route_table" "asav_rt" {
  for_each            = local.subnet_list
...
resource "azurerm_subnet_route_table_association" "asav_rta" {
  for_each       = local.subnet_list
  depends_on     = [azurerm_route_table.asav_rt, azurerm_subnet.subnets]
  subnet_id      = azurerm_subnet.subnets[each.key].id
  route_table_id = azurerm_route_table.asav_rt[each.key].id
}
...
resource "azurerm_network_interface" "asav-diagnostic" {
  depends_on          = [azurerm_subnet.subnets]
...
resource "azurerm_network_interface_security_group_association" "asav_DIAG_NSG" {
  count                     = var.instances
  network_interface_id      = azurerm_network_interface.asav-diagnostic[count.index].id
  network_security_group_id = azurerm_network_security_group.allow-all.id
}
