locals {
  vn_name = var.create_vn ? azurerm_virtual_network.fmcv[0].name : var.vn_name
  subnet_list = {
    "management" = 0
    "diagnostic" = 1
    "outside"    = 2
  ...
  vn_cidr         = var.create_vn ? var.vn_cidr : data.azurerm_virtual_network.fmcv[0].address_space[0]
