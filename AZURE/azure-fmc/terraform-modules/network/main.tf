...
resource "azurerm_virtual_network" "fmcv" {
...
resource "azurerm_route_table" "fmcv_rt" {
  for_each            = local.subnet_list
...
resource "azurerm_subnet_route_table_association" "fmcv_rta" {
  for_each       = local.subnet_list
  depends_on     = [azurerm_route_table.fmcv_rt, azurerm_subnet.subnets]
  subnet_id      = azurerm_subnet.subnets[each.key].id
  route_table_id = azurerm_route_table.fmcv_rt[each.key].id
}
...
resource "azurerm_public_ip" "fmcv-mgmt-interface" {
...
resource "azurerm_network_interface" "fmcv-mgmt" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = azurerm_public_ip.fmcv-mgmt-interface[count.index].id
  }
}


resource "azurerm_network_interface_security_group_association" "fmcv_MGMT_NSG" {
  count                     = var.instances
  network_interface_id      = azurerm_network_interface.fmcv-mgmt[count.index].id
  network_security_group_id = azurerm_network_security_group.allow-all.id
}

resource "azurerm_network_interface" "fmcv-diagnostic" {
  depends_on          = [azurerm_subnet.subnets]
...
resource "azurerm_network_interface_security_group_association" "fmcv_DIAG_NSG" {
  count                     = var.instances
  network_interface_id      = azurerm_network_interface.fmcv-diagnostic[count.index].id
  network_security_group_id = azurerm_network_security_group.allow-all.id
}

resource "azurerm_network_interface" "fmcv-outside" {
  depends_on          = [azurerm_subnet.subnets]
...
resource "azurerm_network_interface_security_group_association" "fmcv_Outside_NSG" {
  count                     = var.instances
  network_interface_id      = azurerm_network_interface.fmcv-outside[count.index].id
  network_security_group_id = azurerm_network_security_group.elb-allow-all.id
}

resource "azurerm_network_interface" "fmcv-inside" {
  depends_on          = [azurerm_subnet.subnets]
...
resource "azurerm_network_interface_security_group_association" "fmcv_Inside_NSG" {
  count                     = var.instances
  network_interface_id      = azurerm_network_interface.fmcv-inside[count.index].id
